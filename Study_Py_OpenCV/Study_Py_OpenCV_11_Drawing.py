...
# 브레젠험 알고리즘은 실수 연산을 하지 않고 정수 연산으로만 선을 그릴 수 있도록 개발된 알고리즘입니다.
# 브레젠험 알고리즘은 4 연결 방식과 8 연결 방식이 있습니다.
# 4 연결 방식의 경우 선분에 픽셀을 할당할 때 다음에 할당될 위치로 오른쪽, 왼쪽, 위쪽, 아래쪽 영역만 고려합니다.
# 8 연결 방식의 경우 대각선 방향까지 추가돼 총 여덟 개의 위치를 고려합니다.
# 안티 에일리어싱은 영상 신호의 결함을 없애기 위한 기법으로서 이미지나 객체의 가장자리 부분에서 발생하는 계단 현상을 없애고 계단을 부드럽게 보이도록 하는 방식입니다.
# 안티 에일리어싱 방식은 가우스 필터링을 사용하며, 넓은 선의 경우 항상 끝이 둥글게 그려집니다.

# 비트 시프트(Bit Shift)Permalink
...
# dst = cv2.line(src, pt1, pt2, color, thickness, lineType, shift)는 입력 이미지(src)에 시작 좌표(pt1)부터 도착 좌표(pt2)를 지나는 특정 색상(color)과 두께(thickness)의 직선을 그립니다.
...
# dst = cv2.circle(src, center, radius, color, thickness, lineType, shift)는 입력 이미지(src)에 중심점(center)으로부터 반지름(radius)크기의 특정 색상(color)과 두께(thickness)의 원을 그립니다.
# 추가로 선형 타입(lineType), 비트 시프트(shift)를 설정할 수 있습니다.
# 만약, 내부가 채워진 원을 그리는 경우, 두께에 cv2.FILLED을 사용해 내부를 채울 수 있습니다.
...
# dst = cv2.rectangle(src, pt1, pt2, color, thickness, lineType, shift)는 입력 이미지(src)에 좌측 상단 모서리 좌표(pt1)부터 우측 하단 모서리 좌표(pt2)로 구성된 특정 색상(color)과 두께(thickness)의 사각형을 그립니다.
# 추가로 선형 타입(lineType), 비트 시프트(shift)를 설정할 수 있습니다.
...
src = cv2.ellipse(src, (1200, 300), (100, 50), 0, 90, 270, (255, 255, 0), 2)
...
# 기준 좌표는 텍스트 박스 좌측 상단 모서리가 아닌 텍스트 박스 좌측 하단 모서리를 사용할 경우 기준 좌표에 true를 지정합니다.
# 설정된 입력값으로 그려진 문자가 포함된 출력 이미지(dst)을 생성합니다.
src = cv2.putText(src, "INSUNG", (900, 600), cv2.FONT_HERSHEY_COMPLEX, 2, (255, 255, 255), 3)
