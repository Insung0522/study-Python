...
# 열림(Opening)Permalink
# open = dilate(erode(src))
# 팽창 연산자와 침식 연산자의 조합이며, 침식 연산을 적용한 다음, 팽창 연산을 적용합니다.
...
# 그레이디언트(Gradient)Permalink
# gradient = dilate(src) - erode(src)
# 팽창 연산자와 침식 연산자의 조합이며, 열림 연산이나 닫힘 연산과 달리 입력 이미지에 각각 팽창 연산과 침식 연산을 적용하고 감산을 진행합니다.
...
src = cv2.imread('Study_Py_OpenCV/src/office.jpg')
