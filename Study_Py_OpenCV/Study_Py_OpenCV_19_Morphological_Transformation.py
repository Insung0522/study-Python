...
# 팽창(Dilation)Permalink
# dilate(x, y) = \max_{\rm (i, j)\in kernel } src(x+i, y+j)
...
# 침식(Erosion)Permalink
# erode(x, y) = \min_{\rm (i, j)\in kernel } src(x+i, y+j)
...
src = cv2.imread('Study_Py_OpenCV/src/zebra.jpg')
...
# Tip : axis=0으로 사용할 경우, 세로 방향으로 연결됩니다.
